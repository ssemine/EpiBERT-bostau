...
alias trim_galore="./~/.local/bin/TrimGalore-0.6.10/trim_galore"
...
# trimming
trim_galore --paired --atac -o "$OUTPUT_DIR" "$R1" "$R2"

R1_TRIM="$OUTPUT_DIR/${sample}_R1_val_1.fq.gz"
R2_TRIM="$OUTPUT_DIR/${sample}_R2_val_2.fq.gz"

# align
bowtie2 -p 8 -x "$BOWTIE2_IDX" -1 "$R1_TRIM" -2 "$R2_TRIM" -S "$OUTPUT_DIR/$sample.sam" \
