...
def main():
    ...
    exons_bed = tb.open(args.exons_bed)
    ...
    for k, subset in enumerate(['valid']):
        ...
        with tf.io.TFRecordWriter(tfr_file, options=tf_opts) as writer:
            for index, row in sub_interval.iterrows():
                ...
                ### exons processing ######################################-
                exons_subints = exons_bed.query(chrom,
                                                interval_start,
                                                interval_end)
                exons_subints_df = pd.DataFrame([rec for rec in exons_subints])
                if (len(exons_subints_df.index) == 0):
                    exons_bedgraph = np.array([0.0] * (args.num_bins))
                else:
                    exons_bedgraph.columns = ['chrom', 'start', 'end']

                    exons_subints_df['start'] = exons_subints_df['start'].astype('int64') - int(interval.start)
                    exons_subints_df['end'] = exons_subints_df['end'].astype('int64') - int(interval.start)
                    exons_subints_df = utils.get_TSS_token(interval, exons_subints_df,
                                                         args.num_bins,args.output_res,args.shift_amt)
                exons_processed = exons_subints_df
                ...
                if np.sum(tss_tokens_gene_spec) > 0:
                    ...
                    feature = {'atac': _tensor_feature_float(atac_processed),
                                ...
                                'exons': _tensor_feature_int(exons_processed),
